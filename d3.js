d3 = function() {
  ...
  function d3_geo_resample(project) {
    ...
    function resampleLineTo(x0, y0, λ0, a0, b0, c0, x1, y1, λ1, a1, b1, c1, depth, buffer) {
      var x01 = x1 - x0, y01 = y1 - y0, d1 = x01 * x01 + y01 * y01;
      if (d1 > 4 * minArea && depth--) {
        var a = a0 + a1, b = b0 + b1, c = c0 + c1, m = Math.sqrt(a * a + b * b + c * c), φ2 = Math.asin(c /= m), λ2 = Math.abs(Math.abs(c) - 1) < ε ? (λ0 + λ1) / 2 : Math.atan2(b, a), p = project(λ2, φ2), x2 = p[0], y2 = p[1], x02 = x0 - x2, y02 = y0 - y2, d2 = Math.abs(x02 * y01 - x01 * y02), tooFar = false;
        if (d2 > minArea || (tooFar = x02 * x02 + y02 * y02 > 256 * minArea)) {
